...
class Editor{
    constructor(response){
        ...
        this.companyName = this.QuerySelector(".jc01").map((i, el) => this.QuerySelector(el).text().trim()).get();
        this.titleName = this.QuerySelector(".job_title").map((i, el) => this.QuerySelector(el).text().trim()).get();

        this.carrerInfo= this.GetChildArr(".job_txt",1);
        this.locationInfo =this.GetChildArr(".job_txt",3);
        this.salaryAmount=this.QuerySelector(".jc03").map((i, el) => this.QuerySelector(el).text().trim()).get();
        this.uploadDate=this.QuerySelector(".jc04").map((i, el) => this.QuerySelector(el).text().trim()).get();
        this.expireDate=this.QuerySelector(".jc05").map((i, el) => this.QuerySelector(el).text().trim()).get();
        this.providedBy =this.QuerySelector(".jc06").map((i, el) => this.QuerySelector(el).text().trim()).get();
